...
function Manager (server, options) {
  ...
  this.settings = {
    ...
    , authorization: false
    , transportOverridesForHttpPorts : null
...
Manager.prototype.initStore = function () {
  ...
  this.store.subscribe('disconnect-remote', function (id, reason) {
    ...
    if (transport) {
      transport.onDisconnect(id, reason);
    }
...
Manager.prototype.handleClient = function (data, req) {
  ...
  if (handshaken) {
    if (transport.open && (data.transport == 'xhr-polling' || !this.connected[data.id])) {
...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  var httpRequestPort = null;
  var host = req.headers['host'];
  if(host)
  {
  	var parts = host.split(':');
  	if(parts.length == 2)
  		httpRequestPort = parts[1];
  }
    ...
    if (authorized) {
      var id = self.generateId()
        , hs = [
            ...
            , self.transports(data, httpRequestPort).join(',')
          ].join(':');
...
Manager.prototype.transports = function (data, urlPort) {
  var transp = this.get('transports')
    , ret = [];
  
  var transportPortOverrides = this.get('transportOverridesForHttpPorts')
  var transportOverrides = null;
  if(urlPort && transportPortOverrides && transportPortOverrides[urlPort] != null)
  {
  	transportOverrides = transportPortOverrides[urlPort];
  }
  for (var i = 0, l = transp.length; i < l; i++) {
    var transport = transp[i];
    if (transport) {
      if (!transport.checkClient || transport.checkClient(data)) {
      	if(!transportOverrides || transportOverrides.indexOf(transport) > -1)
        	ret.push(transport);
      }
    }
  }
